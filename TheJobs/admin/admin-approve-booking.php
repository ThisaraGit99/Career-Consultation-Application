  ...
  if(isset($_POST['approve_booking']))
    {
           ...
           //$c_n_phone  = $_POST['c_n_phone'];
            ...
            $query="update jobs_user set  c_book_status=? where u_id=?";
            $stmt = $mysqli->prepare($query);
            $rc=$stmt->bind_param('si',  $c_book_status, $u_id);
            $stmt->execute();
                if($stmt)
                {
    ...
    <div id="content-wrapper">
      ...
      <?php if(isset($succ)) {?>
        ...
        <div class="card-body">
          ...
          <?php
            $aid=$_GET['u_id'];
            $ret="select * from jobs_user where u_id=?";
        ...
        ?>
          <form method ="POST"> 
            ...
            <div class="form-group">
                <label for="exampleInputEmail1">Consultant Nick Name</label>
                <input type="email" readonly value="<?php echo $row->c_n_name;?>" class="form-control" name="c_n_name">
            </div>

            <div class="form-group">
                <label for="exampleInputEmail1">Consultant Contact</label>
                <input type="email" readonly value="<?php echo $row->c_n_phone;?>" class="form-control" name="u_car_category">
            </div>

        
            <div class="form-group">
                <label for="exampleInputEmail1">Booking Date</label>
                <input type="text" readonly value="<?php echo $row->c_bookdate;?>" class="form-control" id="exampleInputEmail1"  name="c_bookdate">
            </div>

            <div class="form-group">
              <label for="exampleFormControlSelect1">Booking Status</label>
              <select class="form-control" name="c_book_status" id="exampleFormControlSelect1">
